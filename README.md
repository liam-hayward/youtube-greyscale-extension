...
- **Scoped effect** — only the video player is greyed out; the sidebar, subscriptions, and UI chrome remain unaffected
...
├── style.css          # CSS filter scoped to the video player
├── icon16.png
├── icon48.png
└── icon128.png
```
...
3. **`style.css`** applies `filter: grayscale(100%)` to `#movie_player` and `video` elements only, leaving the rest of the YouTube UI intact

---

## Contributing

Pull requests are welcome! If you find a bug or have a feature suggestion, please [open an issue](https://github.com/your-username/youtube-greyscale-toggle/issues).
